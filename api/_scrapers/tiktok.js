...
async function scrapeTikTok(url) {
  ...
  if (d.hdplay) {
    downloads.push({ label: 'Video HD (No Watermark)', url: d.hdplay, type: 'video', ext: 'mp4', quality: 'hd', size: d.hd_size || null });
  }
  if (d.play) {
    downloads.push({ label: 'Video (No Watermark)', url: d.play, type: 'video', ext: 'mp4', quality: 'sd', size: d.size || null });
  }
  if (d.wmplay) {
    downloads.push({ label: 'Video (With Watermark)', url: d.wmplay, type: 'video', ext: 'mp4', quality: 'wm', size: d.wm_size || null });
  }
  if (d.music) {
    downloads.push({ label: `Audio MP3 – ${d.music_info?.title || 'Musik'}`, url: d.music, type: 'audio', ext: 'mp3', quality: 'audio', size: d.music_size || null });
  }
